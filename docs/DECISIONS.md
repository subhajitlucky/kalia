...
| D22 | 2026-09-23 | Adopt the "forbidden multipliers" research track: test-time training (in-place fast weights), self-teaching (EMA teacher), retrieval at deployment. Refuse benchmark leakage and unlabelled claims | Chase a mythical 100× algorithm; ignore test-time compute | Measured multipliers are 1.5–8× (data-side mostly); TTT/Titans is the only technique that gives small models orders-of-magnitude on memory-bound tasks | active |
